...
def all_grid_neighbours(grid):
    """
    Gets all the possible neighbours for each position
    in the grid
    """
    neighbours = {}
...
def path_to_word(grid, path):
    """
    Add all letters on the path inside the grid to a string
    """
    return ''.join([grid[p] for p in path])
    
def search(grid, dictionary):
    """
    search through paths to locate words by matching
